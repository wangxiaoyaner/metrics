 ...
 * <code>createList</code>, and <code>createNewInputObject</code> framework
 * methods.
 * </p>
 * Note: I had to copy this because the original does not give an opportunity to
 * remove the new/remove buttons
 */
public abstract class ListUpDownEditor extends FieldEditor {
	...
	private void createButtons(Composite buttonBox) {
		...
		toggleButton = createPushButton(buttonBox, "ListEditor.toggle");
	}
	...
	/**
	 * Swaps the currently selected item with another.
	 * 
	 * @param delta
	 *            index of the other item relative to selection
	 */
	...
	private void togglePressed() {
		// TODO not really a toggle at the moment
		int index = list.getSelectionIndex();
		if (index >= 0) {
			list.remove(index);
		}
