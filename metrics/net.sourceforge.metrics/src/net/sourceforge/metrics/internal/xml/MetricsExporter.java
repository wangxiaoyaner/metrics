...
public abstract class MetricsExporter implements Constants {
	...
	private static String[] attributeNames = new String[] { "id", "value",
			"avg", "stddev", "points", "max", "per", "maxhandle" };
	...
	private static String format(double value) {
		if (nf == null) {
			...
			int decimals = MetricsPlugin.getDefault().getPreferenceStore()
					.getInt("METRICS.decimals");
	...
	protected void printMetrics(AbstractMetricSource source,
			XMLPrintStream out, int level) {
		out.indent(level);
		out.println("<Metrics>");
		MetricsPlugin plugin = MetricsPlugin.getDefault();
		String[] names = plugin.getMetricIds();
		// 'name' means metric id
	...
	protected void printAttributes(XMLPrintStream out, Properties attributes) {
		for (String name : attributeNames) {
			...
			if (value != null) {
				out.print(' ');
				out.print(formatAttribute(name));
				out.print("=\"");
				out.print(formatAttribute(value));
				out.print('"');
			}
		}
	}
	...
	protected String formatAttribute(String value) {
	...
	protected Properties getAttributes(AbstractMetricSource source, String name) {
		...
		for (String per : PER_ARRAY) {
			...
			Log.logMessage(per);
			if (per.equals(PER_METHOD)) {
				Log.logMessage(source.getParent().getName());
				p.put("class", source.getParent().getName());
			}
		}
		if (p.size() > 0) {
			p.put("id", name);
	 ...
	 * @param i
	 */
	protected void exportChildren(XMLPrintStream out, String handle, int level,
			Class<? extends AbstractMetricSource> filter,
			IProgressMonitor monitor) {
		List<AbstractMetricSource> children = XMLSourceTreeExporter
				.getChildren(handle, filter);
		for (Iterator<AbstractMetricSource> i = children.iterator(); i
				.hasNext();) {
	...
	protected void printOpeningTag(AbstractMetricSource source,
			XMLPrintStream out, int level) {
